...
const bot = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const options = {
      ...
      headers: {
        "x-rapidapi-key": "fe7bda445amsh03b2a65b0602693p1883e3jsnb2827791af3b",
  ...
  return (
    <div className="absolute right-0 bottom-0 m-2 ">
      {toggle ? (
        <div className="rounded-full p-2 text-[35px] bg-white cursor-pointer">
          <div className=" mb-[2px] ml-[1px]">
            <RiRobot3Fill onClick={() => setToggle(!toggle)} />
          </div>
        </div>
      ) : (
            ...
            <div className="text-[12px]">
              {JSON.parse(localStorage.getItem("chat")!)?.map(
                (props: iChat) => (
                  <div key={props?.id}>
                    {props.user === "user" ? (
                      <div className="flex flex-col my-2 p-2 bg-emerald-500 rounded-md ">
                      ...
                      <div className="rounded-md font-[300] italic flex flex-col mb-2 p-2 bg-orange-300">
                        <div className="flex items-center mb-2">
                          <RiRobot3Fill className="text-[30px] mr-[5px]" />
                          <span>Chatbot</span>
                        </div>
                        <div>
                          <div>{props?.message}</div>
